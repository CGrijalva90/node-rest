...
// Instantieate the worker object
const workers = {};
      ...
      checks.forEach(check => {
        ...
        _data.read('checks', check, (err, originalCheckData) => {
          if (!err && originalCheckData) {
            // Pass it to the check validator, and let that unction continue or log errors as needed
...
workers.processCheckOutcome = (originalCheckData, checkOutcome) => {
  ...
  // Save the updates
  _data.update('checks', originalCheckData.id, newCheckData, err => {
    if (!err) {
      // Send the new check data to the nex phase in the process if needed
      if (alertWarranted) {
      ...
      logs.forEach(logName => {
        ...
        _logs.compress(logId, newFileId, err => {
          if (!err) {
            ...
            console.log('Error: Could not compress files');
          }
        });
      });
    } else {
      console.log('Error : Could not find files to rotate');
    }
  });
};
