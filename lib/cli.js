...
cli.responders.help = () => {
  const commands = {
    ...
    'more user info --{userId}': 'Show details of a speciic user',
    'list checks --up --down':
      'Show a list of all the active checks in the system, including their stat. The "--up" and the "--down" flags are optional',
  ...
  // Show a headers for the help page that is as wide as the screen
  cli.horizontalLine();
    ...
    if (!err && checkIds && checkIds.length > 0) {
      ...
      checkIds.forEach(checkId => {
        _data.read('checks', checkId, (err, checkData) => {
          ...
          const stateOrUnkown =
            typeof checkData.state == 'string' ? checkData.state : 'unkown';
...
cli.responders.listLogs = function () {
  _logs.list(true, (err, logFileNames) => {
    if (!err && logFileNames && logFileNames.length > 0) {
      console.log(`logFileNames: ${logFileNames} and length: ${logFileNames.length}`);
      cli.verticalSpace();
