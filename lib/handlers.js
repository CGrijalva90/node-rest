...
handlers.index = (data, callback) => {
  ...
  if (data.method === 'get') {
    ...
    const templateData = {
      ...
      'head.description': 'We offer free, simple uptime montiroing for HTTP/HTTPS sites of all kinds. When your site goes down, we\'ll send you a text to let you know.',
      'body.class': 'index'
    };
...
handlers._checks.post = (data, callback) => {
  ...
  if (protocol && url && method && successCodes && timeoutSeconds) {
        ...
        _data.read('users', userPhone, (err, userData) => {
          if (!err) {
            ...
            if (userChecks.length < config.maxChecks) {
              ...
              callback(400, { Error: `User has reached max checks: ${config.maxChecks}` });
            }
          } else {
    ...
    callback(400, { Error: 'Missing required inputs or inputs are invalid for check' });
  }
};
      ...
      if (!err && checkData) {
        ...
        handlers._tokens.verifyToken(token, checkData.userPhone, tokenIsValid => {
          if (tokenIsValid) {
            ...
            callback(403, { Error: 'Missing required token in heder, or token is invalid' });
          }
        });
      } else {
        ...
        if (!err && checkData) {
          ...
          handlers._tokens.verifyToken(token, checkData.userPhone, tokenIsValid => {
            if (tokenIsValid) {
              ...
              callback(403, { Error: 'Token was not valid! You\'re not authorized' });
            }
          });
        } else {
